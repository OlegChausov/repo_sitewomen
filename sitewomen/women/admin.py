...
class WomenAdmin(admin.ModelAdmin):
    fields = ['title', 'slug', 'content', 'cat', 'husband', 'tags']#поля, отображаемые в меню редактирования поста, порядок как в списке. Все обязательные для модели поля тут должны быть
